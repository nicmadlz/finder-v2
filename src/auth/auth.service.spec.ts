...
describe("AuthService", () => {
    ...
    it("must throw ConflictException if email already exist", async () => {
        jest.spyOn(authService["userRepository"], "findOne").
            mockResolvedValue({ id: "1", email: "test@test.com" } as UserEntity);

        await expect(
            authService.registerUser({ name: "Nicolas", email: "test@test.com", password: "123456" })
    ...
    it("must throw UnauthorizedException if password is wrong", async () => {
        ...
        await expect(
            authService.loginUser({ email: "test@test.com", password: "wrongPassword" })
        ).rejects.toThrow(UnauthorizedException);
    })
    ...
    it("must return user", async () => {
        ...
        jest.spyOn(authService["userRepository"], "save")
            .mockResolvedValue({ id: "1", name: "Nícolas", email: "test@test.com", password: "hashedPassword" } as UserEntity);

        await expect(
            authService.registerUser({ name: "Nícolas", email: "test@test.com", password: "123456" })
